...
for a in tableCheck:
    ...
    if not tableExist:
        mycursor.execute(
            "CREATE TABLE `ecoeatsusers`.`users` (`id` INT NOT NULL, `username` VARCHAR(45) NULL, `password` VARCHAR(45) NULL, PRIMARY KEY (`id`)); ")
...
def create_user():
    ...
    if request.method == 'POST' and create_user_form.validate():
        try:
            ...
            print(
                f"{user.get_userCount()}{user.get_username()} {user.get_password()} was stored in the database successfully.")
...
@app.route('/retrieveReviewsAdmin')
def retrieve_reviews_admin():
    select_query = "SELECT * FROM reviews"
    mycursor.execute(select_query)
    reviews = mycursor.fetchall()

    return render_template('retrieveReviewsAdmin.html', reviews=reviews)
...
def create_membership():
    ...
    if request.method == 'POST' and create_membership_form.validate():
        ...
        membershipFormData = Membership.Membership(create_membership_form.email.data,
                                                   create_membership_form.date_joined.data,
