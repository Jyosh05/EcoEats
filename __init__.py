...
from Forms import CreateUserForm, CreateMembershipForm, CreateReviewsForm, UpdateUserForm, CreateRewardsForm, UpdateRewardsForm
...
def update_reward(id):
    ...
    if request.method == 'POST' and update_reward_form.validate():
        try:
            ...
            select_query = "SELECT reward_name, reward_value, reward_type FROM storerewards WHERE id = %s"
            mycursor.execute(select_query, (id,))
            ...
            if reward_details:
                ...
                update_query = "UPDATE memberships SET reward_name = %s, reward_value = %f, reward_type = %s WHERE id = %s"
                data = (reward_name, reward_value, reward_type, id)
                mycursor.execute(update_query, data)
                mydb.commit()

                print(f"Reward ID: {id} updated successfully.")
                return redirect(url_for('retrieve_reward'))
            else:
        ...
        try:
            ...
            if reward_details:
                ...
                return render_template('updateUser.html', form=update_reward_form)
            else:
                return "Reward not found."
        except Exception as e:
            print("Error:", e)
